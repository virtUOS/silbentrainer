...
const ngrok = "https://74b76b4a.ngrok.io/";
const filepath = "/Users/ek/Projects/dasmitdersilbe/";
//const filepath = "/home/student/e/eukaiser/Projects/dasmitdersilbe/";
...
const start_handlers = Alexa.CreateStateHandler("_START", {
  "Start": function () {
    if(this.attributes["unhandled"]){
      ...
      .listen("Sage lernen, üben oder Lieder hören").audioPlayerStop();
      this.emit(":responseReady");
    } else {
      this.attributes["unhandled"] = false;
...
const lection_handlers = Alexa.CreateStateHandler("_LECTION", {
  'Lection': function () {
    db.getChapterFromUser( this.event.session.user.userId, (chapter) => {
      this.attributes["unhandled"] = false;
      if(chapter == 1){
        this.response.speak("Du bist am Anfang, sage fortfahren um zu starten").listen("Möchtest du das Kapitel fortfahren oder ein bestimmtes Kapitel hören?");
      } else {
        this.response.speak("Du hast Kapitel: "+parseInt(chapter-1)+" gehört, Möchtest du das Kapitel fortfahren oder ein bestimmtes Kapitel hören?").listen("Möchtest du das Kapitel fortfahren oder ein bestimmtes Kapitel hören?");
      }
      this.emit(":responseReady");
    });
  },
  'LectureNumberIntent': function () {
    if(this.attributes["unhandled"]){
      ...
      if(this.event.request.intent.name == "GameContinueIntent"){
        ...
        this.response.speak("Das hat nicht geklappt, aktuell sind 2 Kapitel verfügbar.").listen("Sage zum Beispiel Kapitel Zwei oder Kapitel fortfahren.");
        this.emit(":responseReady");
      }
      ...
      this.attributes["unhandled"] = false;
      this.attributes["continuelecture"] = false;
      this.response.speak("Mit welchem Kapitel möchtest du starten?").listen("Sage zum Beispiel Kapitel Zwei.");
      this.emit(":responseReady");
    }
  },
    ...
    db.getChapterFromUser( this.event.session.user.userId, (chapter) => {
      ...
      if(this.attributes["continuelecture"]){
        if(chapter >1){
          ...
          //db.updateChapter(this.event.session.user.userId, (parseInt(chapter)+1));
          console.log(ngrok+chapter);
          this.response.speak("Kapitel "+chapter+". <audio src='"+ngrok+chapter+"' /> möchtest du mit den Übungen fortfahren?")
          .listen("möchtest du mit den Übungen fortfahren?");
          this.emit(":responseReady");

        } else {
          this.attributes["continuelecture"] = false;

          db.updateChapter(this.event.session.user.userId, 2);

          this.response.speak("Kapitel 1. <audio src='"+ngrok+"1' /> möchtest du mit den Übungen fortfahren?")
          .listen("möchtest du mit den Übungen fortfahren?");
          this.emit(":responseReady");
        ...
        if(this.event.request.intent.slots.Lection){
          ...
          if (this.event.request.intent.slots.Lection != ""){
            ...
            this.response.speak("Kapitel "+hf.getNumberToStringValue(this.event.request.intent.slots.Lection.value)+". <audio src='"+ngrok+hf.getNumberToStringValue(this.event.request.intent.slots.Lection.value)+"' /> möchtest du mit den Übungen fortfahren?")
            .listen("möchtest du mit den Übungen fortfahren?");
            this.emit(":responseReady");
          ...
          if (hf.getNumberToStringValue(this.event.request.intent.slots.Answer.value.toLowerCase()) != ""){
            ...
            //db.updateChapter(this.event.session.user.userId, hf.getNumberToStringValue(this.event.request.intent.slots.Answer.value.toLowerCase()));

            this.response.speak("Kapitel "+hf.getNumberToStringValue(this.event.request.intent.slots.Answer.value.toLowerCase())+". <audio src='"+ngrok+hf.getNumberToStringValue(this.event.request.intent.slots.Answer.value.toLowerCase())+"' /> möchtest du mit den Übungen fortfahren?")
            .listen("möchtest du mit den Übungen fortfahren?");
            this.emit(":responseReady");
        ...
        if( this.attributes["currentQuestion"] === undefined ) {
          ...
          if(this.attributes["randomgame"]){
            ...
            if(chapter == 0 || chapter == null){
              ...
              this.attributes["allQuestions"] = hf.getChapterQuestions(lectures, chapter-1);
            }
          }
            ...
            if(this.event.request.intent.name == "LecturePlayIntent"){
              ...
              (this.event.request.intent.slots.Lection.value.toLowerCase() === "eine" &&  this.attributes["currentQuestion"].answer.toLowerCase() === "eins") ){
                ...
                if(!this.attributes["currentQuestion"]){
                  this.response.speak("Richtig, Du hast deine Aufgaben beantwortet. Bis zum nächsten mal");
                  this.emit(":responseReady");
                } else {
                ...
                if(!this.attributes["currentQuestion"]){
                  this.response.speak("Leider falsch, Du hast deine Aufgaben beantwortet. Bis zum nächsten mal");
                  this.emit(":responseReady");
                } else {
              ...
              if(this.event.request.intent.slots){
                console.log(this.event.request.intent.slots.Answer.value.toLowerCase());
                ...
                this.response.speak('Das hat leider nicht geklappt, versuche es nochmal.').listen('Das hat leider nicht geklappt, versuche es nochmal.');
                this.emit(":responseReady");
              }
